...
$document->addScript('_ferramentas/calendario/js/jquery.smartWizard-2.0.js');
$document->addScript('_ferramentas/calendario/js/jquery.meio.mask.js');
$document->addScript('_ferramentas/calendario/js/detalhes.js');
$document->addScript('http://malsup.github.io/jquery.blockUI.js') ;
$document->addStyleSheet('_ferramentas/calendario/styles/smart_wizard_vertical.css');
$document->addStyleSheet('_ferramentas/calendario/styles/main.css');
...
//SE O VALOR FOR ALTERADO CARREGA DO SERVICE, SENÃO DA CONFIGURAÇÃO
if(isset($_POST['valor_treino'])) {
    $valor_1 = $_POST['valor_treino'] ;
    $valor_2 = $_POST['valor_treino'] ;
} else {
...
switch($treino) {
	Case 'M1':
		$template_main = file_get_contents('_ferramentas/calendario/treino_ui.html') ;    
		break;
	Case 'IF':
		$template_main = file_get_contents('_ferramentas/calendario/treino_ui_if.html') ;    
		break;
	Case 'MTA':
		$template_main = file_get_contents('_ferramentas/calendario/treino_ui_if.html') ;    
		break;
    Case 'MC':
        $template_main = file_get_contents('_ferramentas/calendario/treino_ui_mc.html') ;    
        break ;
}


    $use_mp = (isset($_GET['use_mp']))? 'block':'none' ;
