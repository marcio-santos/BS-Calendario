<?php
    ...
    function create_detail($userid,$cpf,$nome,$email,$sexo,$nascimento,$cep,
        ...
        file_put_contents('query_cadastro.log',$query."\n",FILE_APPEND);
    ...
    function prospect($nome,$cpf,$sexo,$nascimento,$cep,$logradouro,$compl,
        $numero,$bairro,$cidade,$estado,$email,$fone,$celular) {
            ...
            file_put_contents('dump_evo_prospect.log',$nome."\n".$cpf."\n".$sexo."\n".$nascimento."\n".$cep."\n".$logradouro."\n".$compl."\n".
        $numero."\n".$bairro."\n".$cidade."\n".$estado."\n".$email."\n".$fone."\n".$celular."\n\n",FILE_APPEND) ;
    ...
    function baixarCupom($cupom,$destino,$siteid,$decricao,$evoid,$evento_id) {
        $db = &JFactory::getDBO();
        //EXECUTA O PAGAMENTO COM O CUPOM
        $vDestino = $destino;
        $Item_desc = $descricao;
        $query="UPDATE cupons SET beneficiario=".$siteid.", idcliente=".$evoid.",
        destino=".$db->Quote($vDestino).",destino_id=".$evento_id.", utilizado=True,data_utilizado=now() WHERE cupon=".$db->Quote($cupom) ;

        file_put_contents('query_baixa_cupom.log',$query);
        try {
            $db->setQuery($query);
            $db->Query();
...
EOT;
    ...
    */	
    ...
    if($formaPagto=='cupom') {
        ...
        //DA BAIXA NO CUPOM
        $baixaOK = baixarCupom($cupom,$evento,$siteid,$cnab,$evoid,$eventoid);
        ...
        if($lancadoOK[0]==false || $baixaOK[0]==false) {
            ...
            $ret = "<div class='cadastro' style='margin-left:10px;margin-right:10%;'><span style='font-size:18px;'>".$cupom."</span><br/><p>Sua inscrição foi ativada com sucesso
